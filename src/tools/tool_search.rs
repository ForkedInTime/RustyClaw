...
impl Tool for ToolSearchTool {
    ...
    async fn execute(&self, input: serde_json::Value, _ctx: &ToolContext) -> Result<ToolOutput> {
        ...
        let mut scored: Vec<(usize, &str, &str)> = self.tools_snapshot
            .iter()
            .filter_map(|(name, desc)| {
                let combined = format!("{} {}", name.to_lowercase(), desc.to_lowercase());
                let score: usize = terms.iter().map(|t| if combined.contains(t) { 1 } else { 0 }).sum();
                if score > 0 {
        ...
        if scored.is_empty() {
            return Ok(ToolOutput::success(format!("No tools matched '{}'.", input.query)));
        }
