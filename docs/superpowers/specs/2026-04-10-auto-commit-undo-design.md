...
- **Aider** — auto-commits per turn but uses normal branch commits (pollutes history), has `/undo` but no `/redo`
- **Claude Code** — no auto-commit, no `/undo`
- **Claurst** — no auto-commit, no `/undo`
- **Zed AI** — in-editor undo stack, doesn't cross sessions
